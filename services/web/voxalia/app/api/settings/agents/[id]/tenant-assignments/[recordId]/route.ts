...
function payloadFromForm(formData: FormData) {
  ...
  if (typeof payload.priority === "string") payload.priority = Number.parseInt(payload.priority, 10);
...
export async function POST(
  ...
  const { id: agentId, recordId } = await params;
  ...
  if (placeholderAuthEnabled) {
    return redirectPath(request, "info", `Placeholder mode: assignment ${method === "delete" ? "deleted" : "updated"}.`);
  }

  const response = await fetch(`${API_BASE_URL}/settings/agents/${encodeURIComponent(agentId)}/tenant-assignments/${encodeURIComponent(recordId)}`, {
  ...
  return redirectPath(request, "success", method === "delete" ? "Agent assignment deleted permanently." : "Agent assignment updated successfully.");
}
