...
export async function POST(
  ...
  if (placeholderAuthEnabled) {
    return redirectPath(request, "info", `Placeholder mode: extension assignment ${method === "delete" ? "deleted" : "updated"}.`);
  }

  const response = await fetch(`${API_BASE_URL}/settings/agents/${encodeURIComponent(agentId)}/extension-assignments/${encodeURIComponent(recordId)}`, {
  ...
  return redirectPath(request, "success", method === "delete" ? "Extension assignment deleted permanently." : "Extension assignment updated successfully.");
}
