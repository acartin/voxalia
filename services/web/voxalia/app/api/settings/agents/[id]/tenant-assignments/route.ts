...
export async function POST(
  ...
  if (placeholderAuthEnabled) {
    return redirectPath(request, "info", "Placeholder mode: assignment was not saved to the database.");
  }

  const response = await fetch(`${API_BASE_URL}/settings/agents/${encodeURIComponent(agentId)}/tenant-assignments`, {
    method: "POST",
    headers: {
  ...
  return redirectPath(request, "success", "Agent assignment created successfully.");
}
