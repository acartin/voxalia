...
export async function POST(
  ...
  if (placeholderAuthEnabled) {
    return redirectPath(request, "info", `Placeholder mode: number ${method === "delete" ? "deleted" : "updated"}.`);
  }

  const response = await fetch(`${API_BASE_URL}/settings/tenants/${encodeURIComponent(tenantKey)}/numbers/${encodeURIComponent(recordId)}`, {
  ...
  return redirectPath(request, "success", method === "delete" ? "Number deleted permanently." : "Number updated successfully.");
}
