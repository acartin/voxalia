...
export async function POST(
  ...
  if (placeholderAuthEnabled) {
    return redirectPath(request, "info", "Placeholder mode: contact was not saved.");
  }

  const response = await fetch(`${API_BASE_URL}/settings/tenants/${encodeURIComponent(tenantKey)}/contacts`, {
    method: "POST",
    headers: {
  ...
  return redirectPath(request, "success", "Contact created successfully.");
}
