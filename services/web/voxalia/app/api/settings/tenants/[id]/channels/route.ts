...
export async function POST(
  ...
  if (placeholderAuthEnabled) {
    return redirectPath(request, "info", "Placeholder mode: channel was not saved.");
  }

  const response = await fetch(`${API_BASE_URL}/settings/tenants/${encodeURIComponent(tenantKey)}/channels`, {
    method: "POST",
    headers: {
  ...
  return redirectPath(request, "success", "Channel created successfully.");
}
