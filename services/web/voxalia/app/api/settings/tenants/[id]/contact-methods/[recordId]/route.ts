...
export async function POST(
  ...
  if (placeholderAuthEnabled) {
    return redirectPath(request, "info", `Placeholder mode: contact method ${method === "delete" ? "deleted" : "updated"}.`, payload?.contact_id);
  }

  const response = await fetch(`${API_BASE_URL}/settings/tenants/${encodeURIComponent(tenantKey)}/contact-methods/${encodeURIComponent(recordId)}`, {
  ...
  return redirectPath(request, "success", method === "delete" ? "Contact method deleted permanently." : "Contact method updated successfully.", payload?.contact_id);
}
