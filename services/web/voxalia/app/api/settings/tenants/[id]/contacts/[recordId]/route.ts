...
export async function POST(
  ...
  if (placeholderAuthEnabled) {
    return redirectPath(request, "info", `Placeholder mode: contact ${method === "delete" ? "deleted" : "updated"}.`);
  }

  const response = await fetch(`${API_BASE_URL}/settings/tenants/${encodeURIComponent(tenantKey)}/contacts/${encodeURIComponent(recordId)}`, {
  ...
  return redirectPath(request, "success", method === "delete" ? "Contact deleted permanently." : "Contact updated successfully.");
}
