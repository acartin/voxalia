...
function payloadFromForm(formData: FormData) {
  ...
  if (typeof payload.max_channels === "string") payload.max_channels = Number(payload.max_channels);
...
export async function POST(
  ...
  const response = await fetch(`${ASTERISK_API_BASE_URL}/asterisk/infrastructure/trunks/${encodeURIComponent(id)}`, {
  ...
  return redirectPath(request, "success", method === "delete" ? "SIP trunk deleted successfully." : "SIP trunk updated successfully.");
}
