...
function payloadFromForm(formData: FormData) {
  ...
  if (typeof payload.version === "string") payload.version = Number(payload.version);
...
export async function POST(
  ...
  const response = await fetch(`${ASTERISK_API_BASE_URL}/asterisk/tenants/${encodeURIComponent(tenantKey)}/flows/${encodeURIComponent(id)}`, {
  ...
  return redirectPath(request, "success", method === "delete" ? "Flow deleted successfully." : "Flow updated successfully.");
}
