...
export async function POST(request: Request, { params }: { params: Promise<{ tenantKey: string }> }) {
  ...
  const response = await fetch(`${ASTERISK_API_BASE_URL}/asterisk/tenants/${encodeURIComponent(tenantKey)}/flows`, {
    method: "POST",
  ...
  return redirectPath(request, "success", "Flow created successfully.");
}
