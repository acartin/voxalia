...
export async function POST(request: Request, { params }: { params: Promise<{ tenantKey: string }> }) {
  ...
  const payload = payloadFromForm(await request.formData());

  const response = await fetch(`${ASTERISK_API_BASE_URL}/asterisk/tenants/${encodeURIComponent(tenantKey)}/queue-members`, {
    method: "POST",
  ...
  return redirectPath(request, "success", "Queue member created successfully.");
}
