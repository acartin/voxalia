...
export async function POST(request: Request, { params }: { params: Promise<{ tenantKey: string }> }) {
  ...
  const response = await fetch(`${ASTERISK_API_BASE_URL}/asterisk/tenants/${encodeURIComponent(tenantKey)}/extension-devices`, {
    method: "POST",
  ...
  return redirectPath(request, "success", "Extension device created successfully.");
}
