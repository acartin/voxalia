...
function payloadFromForm(formData: FormData) {
  ...
  if (typeof payload.extension_id === "string") payload.extension_id = Number(payload.extension_id);
  if (typeof payload.penalty === "string") payload.penalty = Number(payload.penalty);
  return payload;
}
...
export async function POST(
  ...
  const payload = method === "delete" ? undefined : payloadFromForm(formData);

  const response = await fetch(`${ASTERISK_API_BASE_URL}/asterisk/tenants/${encodeURIComponent(tenantKey)}/queue-members/${encodeURIComponent(id)}`, {
  ...
  return redirectPath(request, "success", method === "delete" ? "Queue member deleted successfully." : "Queue member updated successfully.");
}
